...
public class PageService(IV1ResDtoMapper resDtoMapper, IPageRepository pageRepository,
    IDbEntityMapper<Page, string, PageEntity, string> pageEntityMapper,
    IUnitOfWork unitOfWork) : IPageService
{
    public async Task<V1PageResDto> CreateAsync(CreatePageRequest request, User user, CancellationToken cancellationToken)
    {
    ...
    public async Task<V1PageResDto> UpdateAsync(string id, User user, UpdatePageRequest request, CancellationToken cancellationToken)
    {
        ...
        foreach (var blockRequest in request.ContentBlocks)
        {
            ...
            if (!string.IsNullOrEmpty(blockRequest.Id) && existingBlocksDict.TryGetValue(blockRequest.Id, out var existingBlock))
            {
    ...
    public async Task<V1PageResDto> RestoreAsync(string id, CancellationToken cancellationToken)
    {
        var page = await pageRepository.GetLastAsync(id, cancellationToken);
        page.Restore();
        var updated = await pageRepository.UpdateAsync(page, cancellationToken);
    ...
    public async Task<V1PageResDto> RollbackToVersionAsync(string id, int targetVersionOrder, string? reason, CancellationToken cancellationToken)
    {
    ...
    public async Task<List<PageVersionInfoDto>> GetVersionHistoryAsync(string id, int limit, CancellationToken cancellationToken)
    {
    ...
    public async Task<PageComparisonResDto> CompareVersionsAsync(string id, int sourceVersion, int targetVersion, CancellationToken cancellationToken)
    {
        var sourcePage = await pageRepository.GetAsync(id, new EntityVersion(sourceVersion, null!), cancellationToken);
        var targetPage = await pageRepository.GetAsync(id, new EntityVersion(targetVersion, null!), cancellationToken);
    ...
    public async Task<V1PageResDto> CopyPageAsync(CopyPageRequest request, User user, CancellationToken cancellationToken)
    {
        ...
        if (request.SourceVersionOrder.HasValue)
        {
            sourcePage = await pageRepository.GetAsync(
                ...
                new EntityVersion(request.SourceVersionOrder.Value, null!), 
                cancellationToken);
        }
        else
        {
    ...
    public async Task<V1PageResDto> ReorderContentBlocksAsync(string id, List<int> newOrders, CancellationToken cancellationToken)
    {
        var page = await pageRepository.GetLastAsync(id, cancellationToken);
    ...
    private async Task<Page?> GetPreviousVersionAsync(string id, int currentVersionOrder, CancellationToken cancellationToken)
    {
        try
        {
            return await pageRepository.GetAsync(id, new EntityVersion(currentVersionOrder - 1, null!), cancellationToken);
        }
        catch (DomainException)
        {
            return null;
